...
**Session ledger declaration:** before the first state transition of this session, declare the ledger mode and record it in the active leaf's Events table. If this session will implement or continue any leaf, open or resume the execution ledger (J-space controller, or the manual five-line ledger with `FALLBACK` recorded) — enabled by default, so an implementation session needs a reason not to. The ledger follows the session, not the leaf: it continues naturally across leaf boundaries, and `NOT_ENABLED` is a recorded waiver with a reason (for example "single-session, no cross-context risk"), never a silent omission. A contract predating a field this workflow now requires is migrated before any state transition in this session; `references/contract-migrations.md` lists each field, the section it belongs in, and the `CLARIFICATION` entry it needs.
...
- For a feature, bug fix, or refactor, observe RED before GREEN. Route to available Superpowers skills for brainstorming, planning, TDD, debugging, review, and verification; otherwise use the portable checks and record `FALLBACK`. `FALLBACK` is a leaf-level fact: whenever a leaf actually completes a step through a portable substitute, record it in that leaf's Events table and handoff capability notes, including when the gap was already known and no call failed in this session. The test is whether that leaf ran degraded, not whether it discovered the gap.
...
- Use Review A for every Leaf's contract/implementation coverage, executed by a reviewer holding no implementation context (a fresh subagent fed only the leaf contract, the spec, and the candidate diff — never the session history); a same-context Review A must record `FALLBACK` naming the lost independence. Record the review's model as `REVIEW_MODEL` on its own axis, not as an independence result. When Review B runs on the same leaf, use another fresh reviewer, not the Review A subagent. Review B is required when any kit-owned trigger is hit: `RVB1` security or authentication, `RVB2` external dependency, `RVB3` public API or public meaning, `RVB4` high-risk or irreversible effect, `RVB5` explicit user request. The set is append-only — a retired trigger keeps its number, so an archived record still resolves. Each project records its judgment for all five once in `.charter/project.md` `### 7.1 Review B policy`, and every leaf cites ids instead of re-arguing them: `REQUIRED` names the ids hit, `NOT_REQUIRED` names the ids considered plus the bounded omission reason. `WAIVED` is only for a triggered review whose reviewer was unavailable and whose bounded waiver the user approved; a leaf that hit no trigger is `NOT_REQUIRED`, not `WAIVED`. When Review B is required, use a different reviewer in a fresh context/process; if unavailable, record `BLOCKED_TOOLING` or that bounded waiver. If the charter's reviewer and freshness method is still `UNDECIDED` at the first `REQUIRED` leaf, stop and ask the user before implementing it. An `RVB` id present in one of those two places and missing from the other is a recorded gap, not a silent pass; `references/default-on-policy.md` describes the shipped-enabled shape this rule and the session ledger share.
