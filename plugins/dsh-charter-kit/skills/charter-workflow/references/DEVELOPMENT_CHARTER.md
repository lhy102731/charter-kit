...
首次代码改动前，把关键设计、接口假设、测试意图和计划写入任务目录或交接文件。使用 `grill-me`/`grilling` 做分支树访谈；没有时使用内置设计访谈并记录 fallback。每个实现类叶的会话维护一个简短 ledger（默认启用，见下方会话 ledger 规则）：
...
**会话 ledger（默认启用）。** Ledger 跟随会话而非叶任务：任何将要实现或继续叶任务的会话，在开始时声明其 ledger 模式，并在首个状态转移前把该模式记入活动叶的 Events 表。默认启用控制器（如 J-space 的 `jspace.py`）：`.jspace/` 是执行现场，每个状态转移点（seam）运行 `seam` 刷新账本，恢复时先 `resume` 读全量账本，跨叶时账本自然延续。豁免必须留痕：`NOT_ENABLED` 不是模板里的自由填写，而是 Events 表中一条带理由的声明（例如"单会话内可完成、无跨上下文风险"）。短任务同样受益：任务中途变长（调试加深、会话被压缩）不需要任何切换动作。`.charter/` 始终是治理事实源：叶任务关闭时把执行账本的 Verified 摘要对账进本合同 Events 表，治理层只认对过账的条目——执行账本因关闭对账而不构成第二事实源。控制器不可用时按上述五段格式手工维护 ledger，并记录 `FALLBACK`。
