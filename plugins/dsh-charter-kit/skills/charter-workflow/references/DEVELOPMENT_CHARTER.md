...
预计跨会话或压缩边界、或包含多阶段的长任务，启用 ledger 控制器（如 J-space 的 `jspace.py`）：`.jspace/` 是执行现场，每个状态转移点（seam）运行 `seam` 刷新账本，恢复时先 `resume` 读全量账本。`.charter/` 始终是治理事实源：叶任务关闭时把执行账本的 Verified 摘要对账进本合同 Events 表，治理层只认对过账的条目——执行账本因关闭对账而不构成第二事实源。控制器不可用时按上述五段格式手工维护 ledger，并记录 `FALLBACK`。
