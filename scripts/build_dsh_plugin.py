...
BUILD_COMMAND = "scripts/build_dsh_plugin.py"
DESTINATION_LABEL = "plugins/dsh-charter-kit/"
...
def build_stage(repository_root: Path, stage_root: Path) -> None:
    ...
    write_generated_marker(
        ...
        "targets/dsh/",
        "The adapter files are copied from that target. The Skill tree is copied from "
        "the repository-root `skills/charter-workflow/`, which is itself generated by "
        "`python scripts/build_codex_plugin.py`, so run the Codex builder first.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.check:
        ...
        for label, _destination in sync_destinations(repository_root):
            print(f"Charter Kit DSH packager: synced {label}")
    else:
        print(
