...
# every one of them: a check that covered only the distribution reported PASS
# while the writeback destination DSH packages was stale.
def sync_destinations(repository_root: Path) -> tuple[tuple[str, Path], ...]:
    root = lexical_absolute(repository_root)
    return (
        ("plugins/charter-kit/", root / DISTRIBUTION_RELATIVE),
        ("skills/charter-workflow/", root / PACKAGE_SKILL_RELATIVE),
        (".codex-plugin/plugin.json", root / LEGACY_MANIFEST_RELATIVE),
    )
...
def validate_sync_destination(repository_root: Path, destination: Path) -> None:
    ...
    declaration is not a generated tree and refreshing it would invent a fourth
...
def ensure_check_match(source_root: Path, destination_root: Path) -> None:
    """Compare every destination this builder writes against fresh bytes.

    The distribution was once the only comparison, so ``--check`` reported PASS
    while the writeback destination was stale - and the DSH builder reads that
    writeback destination as its Skill input, so a stale tree shipped in a second
    package with nothing red anywhere.  The writeback is materialized from the
    same fresh stage the distribution comparison uses, which makes the check
    independent of whatever is currently committed under ``plugins/``.
    """
    ...
    with tempfile.TemporaryDirectory(prefix=f".{PACKAGE_NAME}-check-") as temp_dir:
        ...
        mirror_root = Path(temp_dir) / "writeback"
        mirror_root.mkdir(parents=True, exist_ok=True)
        sync_legacy_snapshot(stage_root, mirror_root)
        legacy_skill = source_root / PACKAGE_SKILL_RELATIVE
        if not path_exists(legacy_skill):
            raise FileNotFoundError(f"missing writeback destination: {legacy_skill}")
        if tree_inventory(mirror_root / PACKAGE_SKILL_RELATIVE) != tree_inventory(legacy_skill):
            raise ValueError(f"{legacy_skill} differs from freshly generated bytes")
        legacy_manifest = source_root / LEGACY_MANIFEST_RELATIVE
        if not is_regular_file(legacy_manifest):
            raise FileNotFoundError(f"missing writeback destination: {legacy_manifest}")
        if (mirror_root / LEGACY_MANIFEST_RELATIVE).read_bytes() != legacy_manifest.read_bytes():
            raise ValueError(f"{legacy_manifest} differs from freshly generated bytes")


def sync_generated_destinations(repository_root: Path) -> None:
    """Refresh every declared destination from its declared source.

    The distribution path comes out of the declaration instead of a second copy
    of the same constant, so the allow-list and the write cannot disagree about
    where a generated tree lives.  ``sync_legacy_snapshot`` then writes the two
    remaining declared destinations from the same constants the declaration uses.
    """

    destinations = dict(sync_destinations(repository_root))
    distribution_root = build_distribution(repository_root, destinations["plugins/charter-kit/"])
...
def main(argv: list[str] | None = None) -> int:
    ...
    mode.add_argument(
        "--check",
        action="store_true",
        help="compare every generated destination against a fresh build",
    )
    mode.add_argument(
        "--sync",
        action="store_true",
        help="refresh every generated destination this builder declares",
    )
    parser.add_argument(
        "--output",
        default=None,
    ...
    try:
        if args.check:
            ...
            if args.output is not None:
                validate_sync_destination(repository_root, output_root)
    ...
    if args.check:
        ...
        for label, _destination in sync_destinations(repository_root):
            print(f"Charter Kit packager: synced {label}")
    else:
